...
import { ISDKConfig } from '@portis/web3'
...
export class Portis extends Connector {
  ...
  private async startListening(configuration: any): Promise<void> {
    ...
    return import('@portis/web3').then(async (m) => {
      this.portis = new m.default(dappId, network)
      ...
      const provider = new Web3Provider(this.portis.provider as unknown as ExternalProvider)
